...
MAX_QUEUE_SIZE = int(os.environ.get("SERVE_HTTP_MAX_QUEUE_SIZE", "0"))
...
class HTTPProxy:
    ...
    def __init__(self, controller_name: str):
        ...
        self.latency_metrics_store = InMemoryMetricsStore()

        # logging autoscaling related configurations
        logger.info("lookback_period_s: %s s", LOOK_BACK_PERIOD_S)
        logger.info("max_queue_size: %s", MAX_QUEUE_SIZE)

        self.time_metric_store = TimeMetricStore()
        self.accept_rate = None

    def _record_latency(self, value: Dict[str, float], send_timestamp: float):
        self.latency_metrics_store.add_metrics_point(value, send_timestamp)

    def _latency_window_average(
            self,
            look_back_period_s: float = LOOK_BACK_PERIOD_S) -> Dict[str, float]:
        current_time = time.time()
        return {
            deployment_name: self.latency_metrics_store.window_average(
                deployment_name, current_time - look_back_period_s)
            for deployment_name in self.latency_metrics_store.data.keys()}
    ...
    async def _get_autoscaling_metrics(self):
        loop = asyncio.get_event_loop()
        client = await loop.run_in_executor(None,
                                            serve.context.get_global_client)
        return await loop.run_in_executor(
            None,
            client.get_autoscaling_metrics)
    ...
    async def __call__(self, scope, receive, send):
        ...
        if route_path == "/-/metrics":
            ...
            for deployment_name, handle in self.prefix_router.handles.items():
                merged_dict[deployment_name][HEAD_QUEUE_LEN_KEY] = \
                    handle.router._replica_set.num_queued_queries
            queue_len_metrics_dict = await self._get_autoscaling_metrics()
            for deployment_name, queue_lens in queue_len_metrics_dict.items():
                merged_dict[deployment_name][QUEUE_LEN_KEY] = queue_lens
            latency_metrics_dict = self._latency_window_average()
            for deployment_name, avg_latency in latency_metrics_dict.items():
                merged_dict[deployment_name][LATENCY_KEY] = avg_latency
            input_rate_dict = self.time_metric_store.hist(
                bin_size_s=1, current_ts=time.time())
            for deployment_name, input_rates in input_rate_dict.items():
                merged_dict[deployment_name][INPUT_RATE_KEY] = input_rates
            return await starlette.responses.JSONResponse(merged_dict)(
                scope, receive, send
            )

        if route_path == "/-/accept_rate":
            http_body_bytes = await receive_http_body(scope, receive, send)
            self.accept_rate = json.loads(http_body_bytes)["rate"]
            logger.info(f"accept rate: {self.accept_rate:.4f}")
            return await starlette.responses.PlainTextResponse()(
                scope, receive, send
            )
        ...
        self.time_metric_store.record(handle.deployment_name, start_time)
        if ((self.accept_rate is not None and random.random() > self.accept_rate) or
                (MAX_QUEUE_SIZE > 0 and handle.router._replica_set.num_queued_queries > MAX_QUEUE_SIZE)):
            status_code = 503
            self.request_error_counter.inc(
                tags={"route": route_path, "error_code": str(status_code)}
            )
            ...
            return await response.send(scope, receive, send)
        ...
        self._record_latency({handle.deployment_name: latency_ms}, end_time)
        logger.info(
            access_log_msg(
                method=scope["method"],
                route=route_prefix,
...
class HTTPProxyActor:
    def __init__(
        ...
        for middleware in http_middlewares:
            self.wrapped_app = middleware.cls(
                self.wrapped_app, **middleware.options)
